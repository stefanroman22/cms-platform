...
2. `npm run build` (vite-react-ssg) exits with code 0 (no TypeScript or build errors).
3. `npx playwright test` exits with code 0 (all E2E specs pass).
4. public/sitemap.xml exists (generated by src/seo/sitemap.gen.ts), public/robots.txt exists, and a JSON-LD block is present on at least the home page.
5. A grep for "framer-motion" in src/ returns zero matches.
6. A grep for "next-intl", "next-i18next", "generateMetadata", "app/[locale]", "next/image", or "NEXT_PUBLIC_" returns zero matches.
7. A grep for "react-i18next" and "vite-react-ssg" each return at least one match.
8. A grep for raw "<img " tags outside src/seo/og.gen.ts returns zero matches.
9. .learnings/conventions.md has at least one entry timestamped today.
10. No page produces horizontal overflow at 375px width (verified by the responsive Playwright test).
11. src/routes.tsx exists and defines the /:locale route prefix; pre-rendered dist/<locale>/ HTML exists for every configured locale.
12. src/i18n/messages/<locale>.json seed files exist for every configured locale and contain no `[XX]`/`[NL]` placeholder strings (seed files mirror the default locale; the CMS auto-translates once connected).
13. The locale switcher Playwright test passes.
```
...
2. `npm run build` (vite-react-ssg) exits with code 0.
3. `npx playwright test` exits with code 0.
4. public/sitemap.xml exists (with hreflang alternates, generated by src/seo/sitemap.gen.ts), public/robots.txt exists, and JSON-LD is present on the home page.
5. Grep for "framer-motion", "next-intl", "next-i18next", "generateMetadata", "next/image", and "NEXT_PUBLIC_" each return zero matches.
6. Grep for raw "<img " tags outside src/seo/og.gen.ts returns zero matches.
7. .learnings/conventions.md has at least one entry from today.
8. No page produces horizontal overflow at 375px width.
9. Running Lighthouse via `npx unlighthouse-ci --site http://127.0.0.1:5173` shows:
   - Performance ≥ 90 (mobile)
   - SEO ≥ 95
   - Accessibility ≥ 95
   - Best Practices ≥ 90
   on every locale × every page that exists in the sitemap.
10. `npx @axe-core/cli http://127.0.0.1:5173/en` and same for every other locale report zero violations.
11. Every page has a unique <title> and meta description per locale (no duplicates within a locale).
12. Every <img> usage in components has an alt attribute and srcset/sizes props.
13. Every visible UI string flows through react-i18next `useTranslation()` + `t("ns.key")` (no hardcoded copy in components).
```
...
2. `npm run build` (vite-react-ssg) exits with code 0.
3. The home page renders at http://127.0.0.1:5173/en without console errors.
...
2. npm run build (vite-react-ssg) exits 0
3. npx playwright test exits 0
4. No 'framer-motion', 'next-intl', 'next-i18next', 'generateMetadata', 'NEXT_PUBLIC_' in the codebase
5. No raw <img tags outside src/seo/og.gen.ts
6. public/sitemap.xml includes every locale x every path (generated by src/seo/sitemap.gen.ts)
7. src/routes.tsx defines the /:locale route prefix; dist/<locale>/ HTML exists per locale
