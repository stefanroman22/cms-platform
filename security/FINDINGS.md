...
**Last full review:** 2026-09-17 · **Reviewer:** multi-agent workflow (14 dimensions, adversarial verification; 34 agents) · **Supabase:** xeluydwpgiddbamysgyu (MCP unavailable this run — code + migration review only) · **Confirmed:** 64 · **Dismissed (false-positive):** 14
...
_Status (updated 2026-09-17): **8 new findings** this review (SEC-057…SEC-064). SEC-046 + SEC-047 newly **fixed**; SEC-007/023/025/026 marked **obsolete** (CI/CD teardown removed the cited files). Supabase-anon fixes (SEC-004/013/033/042/053) re-verified — **no regression**. **1 accepted-risk** (SEC-054), **1 needs-decision** (SEC-039). New top risk: **SEC-057 (high)** — CMS Connector cross-tenant admin write via prompt-injected `project_slug`._

> **Review 2026-09-17 — weekly scheduled run.** Scanned all code changed since the 2026-06-07 baseline
> (37 commits): the new **SEO/GEO** feature (router `seo.py` + `seo_repo.py` + agent), **per-staff
> dynamic booking**, **branded booking emails**, the **CMS Connector branch-protection** changes, and
> the **CI/CD overhaul** (only `codeql.yml`/`promote.yml`/`solver-agent.yml` remain; Dependabot
> disabled). **Cross-tenant authZ on all new project-scoped endpoints is clean** — `seo.py` applies
> `require_project_access` on every mutation, the "admins see all projects" change (`projects.py`) is
> correctly gated on server-side `is_admin`, and `deps.py` `require_project_access` enforces
> `user_id == caller or is_admin`. The 8 new findings cluster in: agent prompt-injection (SEC-057),
> rate-limiting on legacy/new endpoints (SEC-058/059), CI supply-chain integrity (SEC-061), residual
> email-escaping (SEC-060), and defense-in-depth (SEC-062/063/064). **MCP note:** Supabase & Vercel MCP
> servers were unavailable in this headless run — DB/RLS analysis is from the SQL migrations only; live
> `get_advisors` was not re-pulled. See [`review-log.md`](./review-log.md).
...
| [SEC-006](findings/medium.md#sec-006) | medium | Solver Agent auto-commits and force-pushes attacker-influenced file changes to cms-preview, which a single Slack ✅ promotes to client production | `agents/Solver - Issues/repo.py:133-160; finalize.py:105-147` | agents | open (materially mitigated 7ae1b07: force-push removed, prod promote now manual `promote.yml`; residual = unreviewed auto-push to staging) |
| [SEC-007](findings/medium.md#sec-007) | medium | Dependabot auto-merge self-approves and merges minor/major-range bumps without independent review; a compromised dependency can reach master/prod | `.github/workflows/dependabot-auto-merge.yml` (DELETED in 7ae1b07) | ci-workflows | ⛔ obsolete (file removed) |
...
| [SEC-023](findings/low.md#sec-023) | low | Auto-rollback pushes a revert to protected master using GITHUB_TOKEN and opens issues from operator-influenced commit subjects | `.github/workflows/post-deploy-smoke.yml` (DELETED in 7ae1b07) | ci-workflows | ⛔ obsolete (file removed) |
| [SEC-024](findings/low.md#sec-024) | low | Solver workflow uses unpinned (mutable-tag) first-party actions while the rest are SHA-pinned | `.github/workflows/solver-agent.yml:64,66` (`actions/checkout@v4`, `setup-python@v5`) | ci-workflows | open (scraper-ci.yml deleted; harden-runner now SHA-pinned; residual = checkout/setup-python tags) |
| [SEC-025](findings/low.md#sec-025) | low | Dependabot does not cover the scraper or the Solver agent (no automated security PRs) | `.github/dependabot.yml` (DELETED — Dependabot disabled entirely) | deps-supplychain | ⛔ obsolete (residual: no automated vuln bumps for any component now) |
| [SEC-026](findings/low.md#sec-026) | low | Dependabot patch/minor PRs auto-approve + auto-merge with no human review, chaining into auto-merge dev→master to prod | `.github/workflows/dependabot-auto-merge.yml` (DELETED in 7ae1b07) | deps-supplychain | ⛔ obsolete (file removed) |
...
| [SEC-046](findings/info.md#sec-046) | info | Bearer auth path returns a plain dict while the rest of the codebase assumes a UserOut object, creating an authZ-shape fragility | `backend/auth_service/routers/deps.py:59-67` (now returns `UserOut(...)`) | admin-priv | ✅ fixed (b6ca3bb) |
| [SEC-047](findings/info.md#sec-047) | info | Session cookie not rotated to a stronger lifetime on remember-me users after password change | `backend/auth_service/routers/auth.py:145-153` (fresh session minted, remember_me=False) | authn-session | ✅ fixed |
...
| [SEC-050](findings/info.md#sec-050) | info | Backend application security-headers middleware omits Content-Security-Policy by design (relies on edge config) | `backend/auth_service/core/security_headers.py:9,13-30` | secrets-config | open (⚠ edge config is INERT — see SEC-062: `vercel.json` headers dropped by legacy `routes` schema) |
...
| [SEC-055](findings/info.md#sec-055) | info | Widget posts resize messages with wildcard target origin | `frontend/src/app/(widget)/w/[slug]/page.tsx:19` | xss-html | open |

## New findings (2026-09-17 review)

| ID | Sev | Title | Location | Dimension | Status |
|---|---|---|---|---|---|
| [SEC-057](findings/high.md#sec-057) | high | CMS Connector provisions/patches projects by a model-chosen `project_slug` derived from untrusted client website files — cross-tenant admin write via prompt injection | `agents/CMS Connector - Website/scan.py:1050-1052` (sink :568,:738) | agents | open |
| [SEC-058](findings/medium.md#sec-058) | medium | Legacy unauth booking `/availability` & `/slots` lack the per-IP limit every other public read has; `/availability` range is unbounded (single-request CPU DoS) | `backend/auth_service/routers/booking.py:944-963` | ratelimit-dos | open |
| [SEC-061](findings/medium.md#sec-061) | medium | `promote.yml` downloads+executes the gitleaks binary with no checksum/signature, in a job holding a `contents:write` PAT + prod deploy hooks (no harden-runner egress block) | `.github/workflows/promote.yml:40-43` | ci-workflows | open |
| [SEC-059](findings/low.md#sec-059) | low | Authenticated SEO `/translate` has no rate limit and fires unbounded paid DeepL calls per request (cost/quota DoS) | `backend/auth_service/routers/seo.py:247-251` | ratelimit-dos | open |
| [SEC-060](findings/low.md#sec-060) | low | Tenant `accent_color` reaches booking-confirmation email `style` attributes unescaped in `_cta_block` (SEC-045 hex allowlist bypassed) | `backend/auth_service/services/booking_email.py:43-70` | xss-html | open |
| [SEC-062](findings/low.md#sec-062) | low | Backend `vercel.json` security-header block silently dropped by legacy `routes` schema (CSP/HSTS/COOP/CORP not delivered) | `backend/vercel.json:5-21` | secrets-config | open |
| [SEC-063](findings/low.md#sec-063) | low | SEO-GEO `render_check.fetch_raw` fetches arbitrary URLs with no scheme/host allowlist, no metadata-IP block, no redirect revalidation (latent SSRF in agent tooling) | `agents/SEO-GEO Optimizer/render_check.py:18-23` | ssrf-outbound | open |
| [SEC-064](findings/info.md#sec-064) | info | SEO/GEO tables enable RLS but omit the `anon`/`authenticated` REVOKE used by peer hardening migrations (defense-in-depth) | `backend/migrations/2026_06_14_seo_geo.sql:158-167` | supabase-db | open |
