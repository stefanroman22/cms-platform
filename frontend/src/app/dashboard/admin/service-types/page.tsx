...
function fetchServiceTypes(): Promise<ServiceType[]> {
  return fetch("/api/admin/service-types", { credentials: "include", cache: "no-store" }).then(
    (r) => {
...
export default function AdminServiceTypesPage() {
  const {
    ...
    loading,
    error,
  } = useQuery<ServiceType[]>("admin:service-types", fetchServiceTypes, { ttl: 5 * 60 * 1000 });
      ...
      {loading && (
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {[...Array(6)].map((_, i) => (
            <div
              key={i}
              className="h-28 rounded-xl border border-zinc-200 bg-white animate-pulse dark:border-zinc-800 dark:bg-zinc-900"
            />
          ))}
        </div>
      )}
