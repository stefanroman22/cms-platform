...
export function IssueList({ projectSlug, refreshTrigger, isAdmin, currentUserId }: IssueListProps) {
  ...
  async function updateStatus(issueId: string, newStatus: IssueStatus) {
    ...
    try {
      const res = await fetch(`/api/projects/${projectSlug}/issues/${issueId}/status`, {
        method: "PATCH",
        credentials: "include",
        headers: { "Content-Type": "application/json" },
            ...
            return (
              <div key={status}>
                ...
                {sectionIssues.length === 0 ? (
                  <p className="text-xs text-zinc-400 dark:text-zinc-500 pl-1">No issues here.</p>
                ) : (
                  <div
                  ...
                  >
                    {sectionIssues.map((issue) => {
                      ...
                      const isUpdatingStatus = updatingStatusId === issue.id;
                                ...
                                {STATUS_TRANSITIONS[issue.status].map(
                                  ({ label: btnLabel, next }) => (
                                    <button
                                      ...
                                      onClick={() => updateStatus(issue.id, next)}
