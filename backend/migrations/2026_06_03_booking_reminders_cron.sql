...
--
-- APPLY ONLY AFTER the backend with POST /booking/cron/reminders is deployed
-- AND the cron secret exists in Vault (otherwise the job harmlessly POSTs a
-- 403/404 every 5 minutes). Runbook:
--   1. Set BOOKING_CRON_SECRET on the backend (Vercel + backend/.env).
