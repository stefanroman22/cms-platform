...
async def test_create_session_caps_at_5_revokes_oldest(mock_supabase, sessions_module):
    ...
    update_calls = [c for c in mock_supabase.update.call_args_list if c.args and isinstance(c.args[0], dict) and c.args[0].get("revoked") is True]
...
async def test_revoke_session_sets_revoked_true(mock_supabase, sessions_module):
    ...
    update_payloads = [c.args[0] for c in mock_supabase.update.call_args_list if c.args and isinstance(c.args[0], dict)]
