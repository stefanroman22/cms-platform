...
async def test_full_login_logout_cycle_against_real_db(test_user):
    from auth_service.services.sessions import create_session, revoke_session, validate_session
...
async def test_session_sliding_expiry_persists_in_db(test_user):
    ...
    short_expiry = datetime.now(UTC) + timedelta(days=1)
    sb.table("sessions").update({"expires_at": short_expiry.isoformat()}).eq(
...
async def test_password_change_kills_sessions_from_other_devices(test_user):
    from auth_service.services.sessions import create_session, revoke_all_for_user, validate_session
